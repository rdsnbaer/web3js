...
/**
 * Hex encodes the data passed to brc_sign and personal_sign
 *
...
*/
export const outputAddressInfo = (receipt) => {
    if(receipt.Address !== null){
       receipt.Address = Utils.toChecksumAddress(receipt.Address);
    }
    if(receipt.BRC !== null){
        receipt.BRC = outputBigNumberFormatter(receipt.BRC);
    }
    if(receipt.FBRC !== null){
        receipt.FBRC = outputBigNumberFormatter(receipt.FBRC);
    }
    if(receipt.FBalance !== null){
        receipt.FBalance = outputBigNumberFormatter(receipt.FBalance);
    }
    if(receipt.balance !== null){
        receipt.balance = outputBigNumberFormatter(receipt.balance);
    }
    if(receipt.ballot !== null){
        receipt.ballot = outputBigNumberFormatter(receipt.ballot);
    }
    if(receipt.nonce !== null){
        receipt.nonce = outputBigNumberFormatter(receipt.nonce);
    }
    if(receipt.poll !== null){
        receipt.poll = outputBigNumberFormatter(receipt.poll);
    }
    //TODO vote ?
    return receipt;
}
